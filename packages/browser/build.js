    ...
    async.waterfall(
      [
        ...
        function (cb) {
          ...
          async.eachSeries(
            fs.readdirSync(path.join(__dirname, "browser-specific")),
            function (item, _cb) {
              fs.copy(
                path.join(__dirname, "browser-specific", item),
                path.join(__dirname, "src", item),
                _cb
              );
            },
        ...
        function (cb) {
          ...
          b.bundle({ standalone: "Nedb" }, function (err, out) {
            ...
            fs.writeFile(
              path.join(__dirname, "out/nedb.js"),
              out,
              "utf8",
              function (err) {
                if (err) {
        ...
        function (out, cb) {
          ...
          fs.writeFile(
            path.join(__dirname, "out/nedb.min.js"),
            compressedCode.code,
            "utf8",
            cb
          );
